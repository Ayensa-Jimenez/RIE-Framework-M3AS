...
\begin{equation}\label{eq:free_energy}
  E(\bs q, S) = \underbrace{W(\bs q)}_{\text{configurational}}
  \;-\;\underbrace{\bs q\cdot\bs\ell(S)}_{\text{coupling to environment}}.
\end{equation}
The configurational term $W$ is intrinsic and independent of the
...
\begin{proof}
By \cref{prop:biot}, $\bs f\in\partial\Psi(\dot{\bs q})$; the
saturation identity~\eqref{eq:saturation} gives
$\bs f\cdot\dot{\bs q}=\Psi(\dot{\bs q})$, whence $\Xi\equiv 0$ and
$\mathcal D=\Psi(\dot{\bs q})\ge 0$ by positiveness of $\Psi$.
\end{proof}
...
$\mathcal D_{\mathrm{tot}}$. The rate-independent structure therefore
