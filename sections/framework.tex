...
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=3pt]
  ...
  closed subset of a Banach space; throughout we take
  ...
  \end{equation}
  where $W\colon\Q\to\R$ is the \emph{free energy} and
...
the play operator of hysteresis. Its usefulness rests on the following
identity, which is the algebraic signature of $1$-homogeneity.

\begin{lemma}[Saturation identity]\label{lem:saturation}
Let $\Psi\colon\R^n\to[0,+\infty)$ be convex, normalised and
positively $1$-homogeneous. Then for every $\bs v\in\R^n$ and every
...
Combining \cref{prop:biot,lem:saturation} gives, along any solution,
$\bs f(\bs q,S)\cdot\dot{\bs q}=\Psi(\dot{\bs q})$, i.e.\ the
dissipation potential is exactly balanced by the energy release rate at
frozen loading. This identity is the cornerstone of the thermodynamic
interpretation of \cref{sec:thermo}. The proofs of
\cref{prop:biot,lem:saturation} are given in \cref{app:framework}.
